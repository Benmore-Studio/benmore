...
func wrapMiddleware(handler http.Handler, app *App, dev bool) http.Handler {
	...
	// Demo mode gate: password-protect the entire app
	if app.FeatureEnabled("testing") {
		demoPassword := GetAppEnv(app.Dir, "DEMO_PASSWORD")
		if demoPassword != "" {
...
func applySecurityHeaders(w http.ResponseWriter, dev bool, app *App) {
	...
	// `frame-ancestors 'none'` can't be combined with explicit origins
	// (the keyword is exclusive). Listing only the desktop origins gives
	// the same protection against web framing while allowing the
	// desktop's iframe.
	...
	// In host mode, allow the platform domain to frame apps (for builder preview)
	if hostPlatform != nil && platformDomain != "" {
		framePolicy = fmt.Sprintf(
			"frame-ancestors 'self' https://%s %s",
			platformDomain, desktopFrameAncestors,
		)
		xframe = ""
	}
	...
	frameSrc := "'self'"
	for _, cdn := range []string{"https://unpkg.com", "https://cdn.jsdelivr.net"} {
		scriptSrc += " " + cdn
		connectSrc += " " + cdn
	}
...
func servePage(w http.ResponseWriter, r *http.Request, app *App, page *Page, dev bool) {
	...
	if page.Redirect != "" && session != nil {
		if target := matchRoleRedirect(page.Redirect, session.Role); target != "" {
