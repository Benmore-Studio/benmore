...
//
// The primary read path (crud_read.go handleList/handleRead) calls
...
func signedURLApp(t *testing.T) (*App, *http.ServeMux, func()) {
	...
	// Full mux registration starts app workers. Stop them and checkpoint before
	// TempDir cleanup so background SQLite writes cannot recreate journal files.
	return app, buildAppMux(app, true, "http://localhost"), app.Shutdown
}
