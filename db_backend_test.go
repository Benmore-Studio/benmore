...
func TestBuildDBOpenConfigDBSynchronousFull(t *testing.T) {
	t.Setenv("DB_SYNCHRONOUS", "FULL")
	cfg := buildDBOpenConfig("/tmp/example-app", "")
	if !strings.Contains(cfg.DSN, "_synchronous=FULL") {
		t.Fatalf("DB_SYNCHRONOUS=FULL not reflected in DSN: %s", cfg.DSN)
	}
}

func TestBuildDBOpenConfigDBSynchronousExtra(t *testing.T) {
	t.Setenv("DB_SYNCHRONOUS", "EXTRA")
	cfg := buildDBOpenConfig("/tmp/example-app", "")
	if !strings.Contains(cfg.DSN, "_synchronous=EXTRA") {
		t.Fatalf("DB_SYNCHRONOUS=EXTRA not reflected in DSN: %s", cfg.DSN)
	}
}
