...
func TestJobsRecoverOrphanedJobs_RequeuesSafeAndIdempotentOnly(t *testing.T) {
	app := newJobsTestApp(t)
	app.Flows = []Flow{
		{Name: "safe_flow", Idempotent: true},
		{Name: "unsafe_flow"},
	}
	expired := time.Now().Add(-2 * jobsLeaseTTL()).UTC().Format(sqliteDateTimeLayout)
	for _, row := range []struct {
		jobType     string
		flowName    string
		maxAttempts int
	}{
		{"cron", "nightly", 3},    // at-least-once (retry budget) -> requeued
		{"cron", "atmostonce", 1}, // at-most-once -> fail closed (no duplicate side effects)
		{"webhook_subscription", "notes.create", 3},
		{"flow", "safe_flow", 3},
		{"flow", "unsafe_flow", 3},
		{"hook", "hook", 3},
	} {
		if _, err := app.DB.Exec(
			"INSERT INTO _benmore_jobs (job_type, flow_name, payload, status, lease_expires_at, run_at, max_attempts) VALUES (?, ?, '{}', 'running', ?, datetime('now'), ?)",
			row.jobType, row.flowName, expired, row.maxAttempts,
		); err != nil {
			t.Fatalf("insert %s/%s: %v", row.jobType, row.flowName, err)
		}
	}
	...
	want := map[string]string{
		"cron/nightly":                      "pending",
		"cron/atmostonce":                   "failed",
		"webhook_subscription/notes.create": "pending",
		"flow/safe_flow":                    "pending",
		"flow/unsafe_flow":                  "failed",
		"hook/hook":                         "failed",
	}
	rows, err := app.DB.Query("SELECT job_type, flow_name, status FROM _benmore_jobs ORDER BY id")
	if err != nil {
		t.Fatal(err)
	}
	defer rows.Close()
	for rows.Next() {
		var jobType, flowName, status string
		if err := rows.Scan(&jobType, &flowName, &status); err != nil {
			t.Fatal(err)
		}
		key := jobType + "/" + flowName
		if status != want[key] {
			t.Fatalf("%s status = %s, want %s", key, status, want[key])
		}
		delete(want, key)
	}
	if len(want) != 0 {
		t.Fatalf("missing job rows: %v", want)
	}
}
...
// nil-able - the flow step handlers are, on inspection, defensively
...
// after an unrelated flow-step change (defeating the regression test without
