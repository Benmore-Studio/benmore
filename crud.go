...
func handleCreate(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	// check happens here. Parent-table creates (join column absent →
	...
	if rule := memberOfFor(app, table, OpWrite); rule != nil && session != nil && !session.IsAdminBypass() {
		if local := rule.localJoinCol(app, table); local != "id" {
			joinVal := strings.TrimSpace(r.FormValue(local))
			if joinVal == "" {
				httpJSON(w, http.StatusUnprocessableEntity, map[string]any{
					"error": fmt.Sprintf("%s is required: %s is membership-scoped (member-of:%s)", local, table, rule.Table),
				})
				return
			}
...
func handleUpdate(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	// leak to non-members.
	if rule := memberOfFor(app, table, OpUpdate); rule != nil && session != nil && !session.IsAdminBypass() {
		if local := rule.localJoinCol(app, table); local != "id" {
			if v, ok := fields[local]; ok && strings.TrimSpace(v) != "" {
				if !memberOfHas(app, rule, strings.TrimSpace(v), session) {
					http.NotFound(w, r)
					return
				}
			}
