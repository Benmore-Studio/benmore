...
func handleList(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	if !isAdmin && !allowBroaderRows {
		...
		if session != nil {
			...
			useGroupScope := session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key)
...
func handleRead(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	if !session.IsAdminBypass() {
		readUseGroup := session != nil && session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key)
		if readUseGroup {
			aclSQL, aclArgs := aclFilter(table, session.Email, "view")
...
func handleUpdate(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	if !session.IsAdminBypass() {
		oldUseGroup := session != nil && session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key)
		if oldUseGroup {
	...
	if !session.IsAdminBypass() {
		updateUseGroup := session != nil && session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key)
		if updateUseGroup {
			aclSQL, aclArgs := aclFilter(table, session.Email, "edit")
...
func handleDelete(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	var scopeArgs []any
	if !session.IsAdminBypass() {
		deleteUseGroup := session != nil && session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key)
		if deleteUseGroup {
...
func handleBatchUpdate(w http.ResponseWriter, r *http.Request, app *App, table string) {
	...
	if !session.IsAdminBypass() {
		if session != nil && session.EffectiveHasGroup() && app.Group != nil && app.Group.Key != "" && hasColumn(app, table, app.Group.Key) {
			aclSQL, aclArgs := aclFilter(table, session.Email, "edit")
