...
func processNextJob(app *App) {
	...
	// Mark as running
	app.DB.Exec("UPDATE _benmore_jobs SET status = 'running', started_at = datetime('now'), attempts = attempts + 1 WHERE id = ?", id)
...
// CleanOldJobs removes completed jobs older than 7 days.
func CleanOldJobs(db *sql.DB) {
	safeGo("jobs.cleanup", func() {
		for {
			time.Sleep(1 * time.Hour)
			db.Exec("DELETE FROM _benmore_jobs WHERE status = 'completed' AND completed_at < datetime('now', '-7 days')")
		}
	})
}
