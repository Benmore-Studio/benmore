...
var feedbackWebhookSem = make(chan struct{}, 8)

func submitFeedbackWebhook(name string, fn func()) bool {
	select {
	case feedbackWebhookSem <- struct{}{}:
		safeGo(name, func() {
			defer func() { <-feedbackWebhookSem }()
			fn()
		})
		return true
	default:
		log.Printf("feedback: webhook queue full; dropping best-effort delivery")
		return false
	}
}
...
// feedbackOriginHost returns the host the visitor was actually on. The widget
// sends location.host (authoritative - distinguishes a custom domain from the
// benmore subdomain, and dev from prod); r.Host is the server-side fallback for
// an old widget or a non-browser submit.
func feedbackOriginHost(clientHost string, r *http.Request) string {
	if h := strings.TrimSpace(clientHost); h != "" {
		return h
	}
	return r.Host
}

// feedbackStatuses is the SINGLE canonical set of feedback statuses, shared by
// the in-app widget/admin, the MCP tools, the dashboard, and the CLI. One set
// here stops the surfaces drifting - before this they had three different
// vocabularies (the dashboard offered a bogus "new" and rejected "wont_fix"
// that the widget happily wrote, so a wont_fix row couldn't be re-statused
// from the dashboard).
var feedbackStatuses = map[string]bool{
	"open": true, "in_progress": true, "resolved": true, "wont_fix": true,
}

// ValidFeedbackStatus reports whether s is a recognized feedback status.
func ValidFeedbackStatus(s string) bool { return feedbackStatuses[s] }

// feedbackReadAllowed reports whether session may read the feedback list/detail
// in-app. A DRAFT (features.testing) is low-stakes QA, so any logged-in tester
// may browse. A PUBLISHED app (features.feedback, testing off) carries real
// visitors' emails, so reads are admin-only - a regular end-user must not be
// able to GET other people's feedback. Anonymous is never allowed (callers
// return an empty list rather than 401 so the widget's own history panel
// doesn't spam console errors).
func feedbackReadAllowed(app *App, session *Session) bool {
	if session == nil {
		return false
	}
...
// feedbackSubmission is the decoded body of a POST /api/_feedback, shared by
// both submit handlers (demo-token gated and session/anon).
...
// feedbackWebhookPayload builds the ONE canonical TESTING_WEBHOOK_URL payload
// used by EVERY submit path. The two handlers used to send divergent shapes -
// the demo one rich, the session/anon one missing app_id - so a webhook
// consumer couldn't reliably tell which app a row came from. originHost is the
// resolved host (client location.host, or r.Host fallback).
func feedbackWebhookPayload(app *App, id int64, b feedbackSubmission, originHost, appURL string) map[string]any {
	return map[string]any{
		"id":               id,
		"app_id":           app.AppID,
		"external_id":      app.ExternalID,
		"visitor_name":     strings.TrimSpace(b.Name),
		"visitor_email":    strings.TrimSpace(b.Email),
		"page_url":         b.PageURL,
...
// fireFeedbackWebhook delivers payload to webhookURL best-effort: SSRF-guarded,
// bounded concurrency (submitFeedbackWebhook's semaphore), 10s timeout. No-op
// when webhookURL is empty or resolves to a private/internal address.
func fireFeedbackWebhook(webhookURL, webhookToken, name string, payload map[string]any) {
	if webhookURL == "" || isPrivateURL(webhookURL) {
		return
	}
	submitFeedbackWebhook(name, func() {
...
func EnsureFeedbackTables(db *sql.DB) {
	...
	// origin_host records the actual host the visitor was on (location.host:
	// the custom domain / <sub>.benmore.ai / <sub>-dev.benmore.ai). page_url is
	// path-only, so without this a feedback row carried NO domain and could not
	// be attributed to the right site/instance. Idempotent ALTER so it adds to
	// pre-existing feedback tables on boot (old rows stay NULL → "unknown host").
	db.Exec(`ALTER TABLE _benmore_feedback ADD COLUMN origin_host TEXT`)
...
func RegisterDemoRoutes(mux *http.ServeMux, app *App, password string) {
	...
	mux.HandleFunc("POST /api/_feedback", func(w http.ResponseWriter, r *http.Request) {
		...
		host := feedbackOriginHost(body.OriginHost, r)
		result, err := app.DB.Exec(
			`INSERT INTO _benmore_feedback (visitor_name, visitor_email, page_url, origin_host, body, element_selector, screenshot)
			 VALUES (?, ?, ?, ?, ?, ?, ?)`,
			strings.TrimSpace(body.Name), strings.TrimSpace(body.Email),
			body.PageURL, host, strings.TrimSpace(body.Body),
		...
		fireFeedbackWebhook(webhookURL, webhookToken, fmt.Sprintf("feedback.webhook:%d", id),
			feedbackWebhookPayload(app, id, body, host, r.Host))
	...
	mux.HandleFunc("GET /api/_feedback", func(w http.ResponseWriter, r *http.Request) {
		...
		rows, _ := QueryRows(app.DB, `SELECT id, visitor_name, visitor_email, page_url, origin_host, body, element_selector, status, created_at
	...
	mux.HandleFunc("PATCH /api/_feedback/{id}", func(w http.ResponseWriter, r *http.Request) {
		...
		if !ValidFeedbackStatus(body.Status) {
...
// requiring a demo password. Called whenever the feedback widget is enabled
// (features.testing OR features.feedback) so the widget can submit with a
// regular session (or anonymously) and tooling can read from the table.
//
// Read access adapts to mode: on a DRAFT (features.testing) any logged-in
// tester may browse feedback, because a draft is low-stakes QA. On a PUBLISHED
// app (features.feedback, testing off) the list carries real visitors' emails,
// so reads are admin-only - a regular end-user must not be able to GET other
// people's feedback. Anonymous reads always return an empty list (not 401) so
// the widget's own history panel on /login doesn't spam console errors.
func RegisterFeedbackAPI(mux *http.ServeMux, app *App) {
	webhookURL := GetAppEnv(app.Dir, "TESTING_WEBHOOK_URL")
	webhookToken := GetAppEnv(app.Dir, "TESTING_WEBHOOK_TOKEN")

	// POST /api/_feedback - submit. Open to anonymous visitors so people
	// previewing a draft (e.g., on /login) or using a published app can leave
	// feedback without an account. Authenticated users get their email
	// auto-attached.
	mux.HandleFunc("POST /api/_feedback", func(w http.ResponseWriter, r *http.Request) {
		session := getSession(app, r)
		...
		host := feedbackOriginHost(body.OriginHost, r)
		result, err := app.DB.Exec(
			`INSERT INTO _benmore_feedback (visitor_name, visitor_email, page_url, origin_host, body, element_selector, screenshot) VALUES (?, ?, ?, ?, ?, ?, ?)`,
			strings.TrimSpace(body.Name), strings.TrimSpace(body.Email),
			body.PageURL, host, strings.TrimSpace(body.Body),
			body.ElementSelector, body.Screenshot)
		if err != nil {
	...
	// GET /api/_feedback - list. Draft: any session. Published: admin only.
	// Anonymous / unauthorized callers get an empty list (not 401) so the
	// injected widget on /login doesn't spam red errors in the console.
	mux.HandleFunc("GET /api/_feedback", func(w http.ResponseWriter, r *http.Request) {
		if !feedbackReadAllowed(app, getSession(app, r)) {
			httpJSON(w, http.StatusOK, []any{})
			return
		}
		rows, _ := QueryRows(app.DB, `SELECT id, visitor_name, visitor_email, page_url, origin_host, body, element_selector, status, created_at FROM _benmore_feedback ORDER BY created_at DESC`)
		httpJSON(w, http.StatusOK, rows)
	})

	// GET /api/_feedback/{id} - single record + comments. Same read gate.
	mux.HandleFunc("GET /api/_feedback/{id}", func(w http.ResponseWriter, r *http.Request) {
		session := getSession(app, r)
		if session == nil {
		...
		if !feedbackReadAllowed(app, session) {
			httpJSON(w, http.StatusForbidden, map[string]any{"error": "admin required"})
			return
		}
...
// FeedbackWidgetHTML renders the floating feedback widget. testing controls
// only the sidebar intro copy: on a draft/QA preview it names "testing mode";
// on a published app running features.feedback it's neutral ("We'd love your
// feedback") so the widget doesn't tell real end-users the live app is a test.
func FeedbackWidgetHTML(testing bool) string {
	intro := `<strong>We'd love your feedback.</strong> Leave a note any of these ways - click one or use the shortcut key:`
	if testing {
		intro = `<strong>This app is in testing mode.</strong> Leave feedback any of these ways - click one or use the shortcut key:`
	}
...
<div id="bm-feedback-panel" style="display:none;position:fixed;top:0;right:0;z-index:99992;width:380px;max-width:100vw;height:100vh;background:#ffffff;border-left:1px solid #e2e8f0;box-shadow:-8px 0 30px rgba(0,0,0,0.1);font-family:system-ui,-apple-system,sans-serif;overflow-y:auto;transform:translateX(100%);transition:transform 0.25s ease;">
  ...
  <div style="padding:14px 20px;background:#faf5ff;border-bottom:1px solid #e9d5ff;">
    <div style="font-size:12px;color:#6b21a8;line-height:1.55;margin-bottom:10px;">` + intro + `</div>
...
func ServeFeedbackAdmin(w http.ResponseWriter, r *http.Request, app *App) {
	...
	sb.WriteString(`<div style="background:hsl(var(--card));border:1px solid hsl(var(--border));border-radius:8px;overflow:hidden;">
		...
		<thead><tr style="border-bottom:1px solid hsl(var(--border));">
			...
			<th style="text-align:left;padding:10px 16px;font-weight:600;color:hsl(var(--muted-foreground));font-size:11px;text-transform:uppercase;">Host</th>
	...
	for _, row := range rows {
		...
		host := fmt.Sprintf("%v", row["origin_host"])
		if host == "<nil>" || host == "" {
			host = "—"
		}
		...
		sb.WriteString(fmt.Sprintf(`<tr style="border-bottom:1px solid hsl(var(--border)/0.5);cursor:pointer;" onclick="showFeedbackDetail(%s)">
			...
			<td style="padding:8px 16px;color:hsl(var(--muted-foreground));font-size:12px;font-family:ui-monospace,monospace;">%s</td>
			...
			html.EscapeString(name), html.EscapeString(page), html.EscapeString(host), html.EscapeString(body), html.EscapeString(created)))
	}

	if len(rows) == 0 {
		sb.WriteString(`<tr><td colspan="6" style="padding:32px;text-align:center;color:hsl(var(--muted-foreground));">No feedback yet. Enable testing mode (or features.feedback on a live app) so visitors get the feedback widget.</td></tr>`)
	}
	...
	function showFeedbackDetail(id){
		fetch('/api/_feedback/'+id,{credentials:'same-origin'}).then(function(r){return r.json()}).then(function(fb){
			...
			var h='<div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:16px;"><div><div style="font-size:16px;font-weight:600;">Feedback #'+fb.id+'</div><div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:2px;">'+escH(from)+(fb.visitor_email?' &middot; '+escH(fb.visitor_email):'')+' on '+escH(fb.page_url)+'</div></div><button onclick="this.closest(\'dialog\').close()" style="background:none;border:none;cursor:pointer;font-size:20px;color:hsl(var(--muted-foreground));">&times;</button></div>';
			h+='<div style="background:hsl(var(--background));border:1px solid hsl(var(--border));border-radius:8px;padding:16px;margin-bottom:16px;font-size:14px;line-height:1.6;white-space:pre-wrap;">'+escH(fb.body)+'</div>';
			if(fb.element_selector)h+='<div style="font-size:12px;color:hsl(var(--muted-foreground));margin-bottom:12px;">Element: <code style="background:hsl(var(--muted));padding:2px 6px;border-radius:4px;">'+escH(fb.element_selector)+'</code></div>';
			if(fb.screenshot)h+='<img src="'+escH(fb.screenshot)+'" style="max-width:100%%;border:1px solid hsl(var(--border));border-radius:8px;margin-bottom:16px;">';
			...
			(fb.comments||[]).forEach(function(c){h+='<div style="background:hsl(var(--background));border:1px solid hsl(var(--border));border-radius:8px;padding:12px;margin-bottom:8px;"><div style="font-size:11px;color:hsl(var(--muted-foreground));margin-bottom:4px;">'+escH(c.user_email)+' &middot; '+escH(c.created_at)+'</div><div style="font-size:13px;">'+escH(c.body)+'</div></div>';});
