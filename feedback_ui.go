...
func RegisterDemoRoutes(mux *http.ServeMux, app *App, password string) {
	...
	// POST /_demo/login — validate demo password
...
func BuiltWithBadgeHTML(app *App) string {
	...
	// The "Built with Benmore" badge is a hosted-platform free-tier feature.
	// Self-hosted apps have no platform plan (OwnerPlan == ""), so they never
	// get a forced badge; only the hosted free tier shows it.
	if app.OwnerPlan != "free" {
		return ""
	}

	var label string
	if !app.PublishedAt.IsZero() && app.BuildSeconds > 0 {
		label = fmt.Sprintf("Built with Benmore in %s", humanDuration(app.BuildSeconds))
	} else if app.BuildSeconds > 0 {
		label = fmt.Sprintf("Building with Benmore · %s", humanDuration(app.BuildSeconds))
	} else {
		label = "Built with Benmore"
	}
	...
	siteName := ""
	if app.Design != nil && app.Design.SEO != nil {
		siteName = app.Design.SEO["site_name"]
	}
	if siteName == "" && subRef != "" {
		siteName = subRef
	...
	href := "https://benmore.ai/?ref=" + subRef
	inlineLogo := `<img class="bm-bb-logo" src="` + benmoreLogoDataURI + `" alt="Benmore" />`

	// The popover anchored to the badge consolidates three things that
	// previously lived in separate floating chrome (banner, error
	// overlay, build-time badge): the experimental-framework disclaimer
	// (shown on every visit, highlighted on first visit), the testing-
	// mode metadata (current site + user role) when features.testing
	// is enabled, and a slot for the runtime error list that bm.js
	// populates dynamically. Clicking the badge toggles the popover;
	// the original Benmore.ai link moves inside as an explicit row so
	// the badge click stops being navigational.
	showTesting := app.FeatureEnabled("testing")

	return fmt.Sprintf(`<!-- Built with Benmore badge + info popover (free tier) -->
<style>
#bm-built-badge{position:fixed;bottom:16px;right:16px;z-index:99996;display:inline-flex;align-items:center;gap:9px;padding:7px 15px 7px 7px;border-radius:9999px;background:#fff;border:1px solid #e4e4e7;box-shadow:0 2px 8px rgba(0,0,0,0.08);text-decoration:none;color:#18181b;font-family:system-ui,-apple-system,sans-serif;font-size:12px;font-weight:500;line-height:1;white-space:nowrap;transition:all 0.2s;cursor:pointer;border:none;}
#bm-built-badge:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(0,0,0,0.12);}
#bm-built-badge .bm-bb-logo{width:22px;height:22px;flex-shrink:0;display:block;}
#bm-built-badge .bm-bb-text{color:#52525b;}
#bm-info-pop{position:fixed;bottom:60px;right:16px;z-index:99997;display:none;width:340px;max-width:calc(100vw - 32px);background:#fff;color:#18181b;border:1px solid #e4e4e7;border-radius:14px;box-shadow:0 20px 50px rgba(0,0,0,0.18);font-family:system-ui,-apple-system,sans-serif;font-size:13px;line-height:1.5;overflow:hidden;}
#bm-info-pop.open{display:block;animation:bm-info-in 0.16s ease-out;}
@keyframes bm-info-in{from{transform:translateY(8px);opacity:0}to{transform:translateY(0);opacity:1}}
#bm-info-pop .bm-ip-disclaimer{padding:14px 16px;background:linear-gradient(180deg,#fef3c7,#fde68a);border-bottom:1px solid #fcd34d;}
#bm-info-pop .bm-ip-disclaimer .bm-ip-tag{display:inline-block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#92400e;background:rgba(255,255,255,0.6);padding:2px 7px;border-radius:4px;margin-bottom:6px;}
#bm-info-pop .bm-ip-disclaimer p{margin:0;color:#78350f;font-size:12.5px;}
#bm-info-pop .bm-ip-section{padding:12px 16px;border-bottom:1px solid #f4f4f5;}
#bm-info-pop .bm-ip-section:last-child{border-bottom:none;}
#bm-info-pop .bm-ip-h{font-size:10.5px;text-transform:uppercase;letter-spacing:0.08em;color:#71717a;font-weight:700;margin:0 0 6px;}
#bm-info-pop .bm-ip-kv{display:flex;justify-content:space-between;gap:12px;font-size:12.5px;}
#bm-info-pop .bm-ip-kv .bm-ip-k{color:#52525b;}
#bm-info-pop .bm-ip-kv .bm-ip-v{color:#18181b;font-weight:500;text-align:right;word-break:break-word;}
#bm-info-pop .bm-ip-link{display:flex;align-items:center;gap:8px;padding:11px 16px;color:#18181b;text-decoration:none;font-weight:500;font-size:12.5px;background:#fafafa;border-top:1px solid #f4f4f5;transition:background 0.12s;}
#bm-info-pop .bm-ip-link:hover{background:#f4f4f5;}
#bm-info-pop .bm-ip-link svg{width:14px;height:14px;flex-shrink:0;}
@media(max-width:480px){
  #bm-built-badge{bottom:12px;right:12px;padding:6px 13px 6px 6px;font-size:11px;}
  #bm-built-badge .bm-bb-logo{width:20px;height:20px;}
  #bm-info-pop{bottom:54px;right:12px;}
}
@media(prefers-color-scheme:dark){
  #bm-built-badge{background:#0a0a0a;color:#e4e4e7;border:1px solid #27272a;}
  #bm-built-badge .bm-bb-text{color:#a1a1aa;}
  #bm-info-pop{background:#0a0a0a;color:#e4e4e7;border-color:#27272a;}
  #bm-info-pop .bm-ip-section{border-color:#1c1c22;}
  #bm-info-pop .bm-ip-h{color:#71717a;}
  #bm-info-pop .bm-ip-kv .bm-ip-k{color:#a1a1aa;}
  #bm-info-pop .bm-ip-kv .bm-ip-v{color:#fafafa;}
  #bm-info-pop .bm-ip-link{background:#09090b;border-color:#1c1c22;color:#fafafa;}
  #bm-info-pop .bm-ip-link:hover{background:#18181b;}
}
</style>
<button id="bm-built-badge" type="button" aria-label="About this app">
  %s
  <span class="bm-bb-text">%s</span>
</button>
<div id="bm-info-pop" role="dialog" aria-label="Benmore app info" data-app="%s" data-testing="%s">
  <div class="bm-ip-disclaimer">
    <span class="bm-ip-tag">Experimental</span>
    <p>Benmore-Go is an experiment. Treat it carefully for anything live. Features can change, data models may shift between releases, and prod uptime isn't guaranteed.</p>
  </div>
  %s
  <div class="bm-ip-section" data-bm-errors-slot style="display:none;">
    <p class="bm-ip-h" data-bm-errors-title>Runtime errors</p>
    <div data-bm-errors-list></div>
  </div>
  <a class="bm-ip-link" href="%s" target="_blank" rel="noopener">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
    <span>Open Benmore.ai</span>
  </a>
</div>
<script>
(function(){
  var b=document.getElementById('bm-built-badge'),p=document.getElementById('bm-info-pop');
  if(!b||!p)return;
  function open(){p.classList.add('open');}
  function close(){p.classList.remove('open');}
  b.addEventListener('click',function(e){e.stopPropagation();if(p.classList.contains('open'))close();else open();});
  document.addEventListener('click',function(e){if(!p.classList.contains('open'))return;if(p.contains(e.target)||b.contains(e.target))return;close();});
  document.addEventListener('keydown',function(e){if(e.key==='Escape')close();});
})();
</script>
`, inlineLogo, escapeHTMLAttr(label), escapeHTMLAttr(subRef), boolStr(showTesting), buildInfoSection(app, siteName, showTesting), href)
}
