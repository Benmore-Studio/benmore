...
// LoadCron is the compatibility wrapper. Startup and reload propagate parse errors.
...
func parseCronYAML(data []byte) (*CronConfig, error) {
	var shape map[string]any
	if err := decodeConfigYAML(data, &shape); err != nil {
		return nil, fmt.Errorf("cron.yaml: %w", err)
	}
	if len(shape) == 0 {
		return nil, nil
	}
	var jobs []cronJobYAML
	if _, array := shape["jobs"]; array {
		if len(shape) != 1 {
			return nil, fmt.Errorf("cron.yaml: cannot mix jobs list with named jobs")
		}
		if _, ok := shape["jobs"].([]any); !ok {
			return nil, fmt.Errorf("cron.yaml: jobs must be a list")
		}
		var config cronConfigYAML
		if err := decodeConfigYAML(data, &config); err != nil {
			return nil, fmt.Errorf("cron.yaml: %w", err)
		}
		jobs = config.Jobs
	} else {
		var config map[string]cronMapEntry
		if err := decodeConfigYAML(data, &config); err != nil {
			return nil, fmt.Errorf("cron.yaml: %w", err)
		}
		...
		for _, name := range names {
			entry := config[name]
			if entry.SQL != "" {
				if len(entry.Run) > 0 || len(entry.Steps) > 0 || entry.Flow != "" {
					return nil, fmt.Errorf("cron.yaml: %s mixes sql with another action", name)
				}
				entry.Run = []FlowStepYAML{{SQL: entry.SQL}}
			}
			jobs = append(jobs, cronJobYAML{ID: name, Schedule: entry.Schedule, Run: entry.Run, Steps: entry.Steps, Flow: entry.Flow, MaxAttempts: entry.MaxAttempts})
		}
	}
	if len(jobs) == 0 {
		return nil, nil
	...
	for _, j := range jobs {
		if j.ID == "" || j.Schedule == "" {
			return nil, fmt.Errorf("cron.yaml: each job requires id and schedule")
		}
		if err := validateCronSchedule(j.Schedule); err != nil {
			return nil, fmt.Errorf("cron.yaml: %s: %w", j.ID, err)
		}
		if seen[j.ID] {
			return nil, fmt.Errorf("cron.yaml: duplicate job %q", j.ID)
		}
		seen[j.ID] = true
		if len(j.Run) > 0 && len(j.Steps) > 0 {
			return nil, fmt.Errorf("cron.yaml: %s uses both run and steps", j.ID)
		}
		steps := j.Run
		if len(steps) == 0 {
			steps = j.Steps
		}
		if (j.Flow == "") == (len(steps) == 0) {
			return nil, fmt.Errorf("cron.yaml: %s requires either flow or inline steps", j.ID)
		}
		if j.MaxAttempts < 0 {
			return nil, fmt.Errorf("cron.yaml: %s max_attempts must not be negative", j.ID)
		}
		converted := convertSteps(steps)
		if err := checkConvertedSteps(converted); err != nil {
			return nil, fmt.Errorf("cron.yaml: %s: %w", j.ID, err)
		}
		out.Jobs = append(out.Jobs, CronJob{ID: j.ID, Schedule: j.Schedule, Steps: converted, Flow: j.Flow, MaxAttempts: j.MaxAttempts})
	}
	return out, nil
}

// Validate exactly the vocabulary cronShouldFire/cronMatch can execute.
func validateCronSchedule(expr string) error {
	...
	if strings.HasPrefix(expr, "every ") {
		if d, err := time.ParseDuration(strings.TrimPrefix(expr, "every ")); err == nil && d > 0 {
			return nil
		}
		return fmt.Errorf("invalid interval schedule %q", expr)
	}
	fields := strings.Fields(expr)
	if len(fields) != 5 {
		return fmt.Errorf("schedule must have five cron fields or a supported shortcut")
	}
	...
	for i, field := range fields {
		...
		if strings.HasPrefix(field, "*/") {
			...
			return fmt.Errorf("invalid cron step %q", field)
		}
		for _, part := range strings.Split(field, ",") {
			...
			for _, value := range values {
				...
				if err != nil || n < bounds[i][0] || n > bounds[i][1] || n < previous {
					return fmt.Errorf("invalid cron field %q", field)
				}
				previous = n
			}
