...
```bash
go install -tags sqlite_fts5 github.com/benmore-studio/benmore@latest
# or, from a clone:
...
benmore serve myapp --port 8080    # self-hosting service; configure your ingress
```
...
serves it. Operate this as a self-hosted service with appropriate ingress and
process supervision. The hosted development workflow uses the separate cloud
CLI and a deployed dev URL; there is no Benmore localhost development mode.

> The `sqlite_fts5` build tag enables SQLite FTS5 full-text search (used when an
> app declares `@@fulltext`).

The scaffold includes `AGENTS.md` for Codex and other agents and a `CLAUDE.md`
import of the same instructions. Framework guides describe self-hosting; hosted
`push`, `api`, account commands, and skill installation require the cloud CLI.
