...
// isBearerAuth returns true if the request uses Bearer token auth (not cookies).
// Bearer-authenticated requests don't need CSRF protection since there's no cookie to steal.
...
func recordLoginAttempt(db *sql.DB, email string, success bool) {
	if _, err := db.Exec("INSERT INTO _benmore_login_attempts (email, success) VALUES (?, ?)", email, success); err != nil {
...
func handleLogin(w http.ResponseWriter, r *http.Request, app *App) {
	...
	var hash string
	var email string
	// Look up user by configured identifier, also fetch email for session creation
	...
	if app.Design != nil && app.Design.Auth["otp"] == "true" {
		...
		http.Redirect(w, r, app.Paths.VerifyOTP, http.StatusSeeOther)
		return
	}
...
func handleVerifyOTP(w http.ResponseWriter, r *http.Request, app *App) {
	...
	if err != nil || cookie.Value == "" {
		authRedirect(w, r, app.Paths.VerifyOTP, "Session expired. Please log in again.")
		return
	}
	...
	if verified == "" {
		authRedirect(w, r, app.Paths.VerifyOTP, "Invalid session. Please log in again.")
		return
	}
	...
	if err != nil {
		authRedirect(w, r, app.Paths.VerifyOTP, "Code expired. Please log in again.")
		return
	}

	if attempts >= 5 {
		app.DB.Exec("DELETE FROM _benmore_otp WHERE email = ?", email)
		authRedirect(w, r, app.Paths.VerifyOTP, "Too many attempts. Please log in again.")
		return
	}

	if !SecureCompare(code, storedCode) {
		app.DB.Exec("UPDATE _benmore_otp SET attempts = attempts + 1 WHERE email = ?", email)
		authRedirect(w, r, app.Paths.VerifyOTP, fmt.Sprintf("Invalid code. %d attempts remaining.", 4-attempts))
		return
	}
...
func emailAllowedForApp(app *App, email string) (bool, string) {
	...
	domainRaw := strings.TrimSpace(app.Design.Auth["domain"])
	if domainRaw == "" {
		return true, "" // no restriction configured
	}
	email = strings.ToLower(strings.TrimSpace(email))

	// Exception allowlist — exact-address bypass.
	if allowRaw := strings.TrimSpace(app.Design.Auth["allow_emails"]); allowRaw != "" {
...
func handleSignup(w http.ResponseWriter, r *http.Request, app *App) {
	...
	if needsVerification {
		...
		app.DB.Exec("INSERT INTO _benmore_password_resets (token, email, expires_at, type) VALUES (?, ?, ?, 'verify')",
			token, email, expires.UTC().Format(time.RFC3339))
...
func authRedirect(w http.ResponseWriter, r *http.Request, path, errMsg string) {
	...
	if errMsg != "" {
		path += "?error=" + url.QueryEscape(errMsg)
	}
...
func handleForgotPassword(w http.ResponseWriter, r *http.Request, app *App) {
	...
	var count int
	app.DB.QueryRow("SELECT COUNT(*) FROM _benmore_users WHERE email = ?", email).Scan(&count)
	if count > 0 {
		token := generateToken(32)
		expires := time.Now().Add(1 * time.Hour)
...
func handleResetPassword(w http.ResponseWriter, r *http.Request, app *App) {
	...
	var email string
	err := app.DB.QueryRow(
		"SELECT email FROM _benmore_password_resets WHERE token = ? AND expires_at > datetime('now') AND used = 0 AND type = 'reset'",
		token,
	).Scan(&email)
	if err != nil {
	...
	result, _ := app.DB.Exec("UPDATE _benmore_password_resets SET used = 1 WHERE token = ? AND used = 0", token)
	...
	app.DB.Exec("UPDATE _benmore_users SET password_hash = ? WHERE email = ?", string(hash), email)

	// Kill ALL existing sessions for this user — attacker sessions die here
	var userID int64
	app.DB.QueryRow("SELECT id FROM _benmore_users WHERE email = ?", email).Scan(&userID)
	if userID > 0 {
...
func handleVerifyEmail(w http.ResponseWriter, r *http.Request, app *App) {
	...
	var email string
	err := app.DB.QueryRow(
		"SELECT email FROM _benmore_password_resets WHERE token = ? AND expires_at > datetime('now') AND used = 0 AND type = 'verify'",
		token,
	).Scan(&email)
	if err != nil {
		http.Redirect(w, r, app.Paths.Login, http.StatusSeeOther)
	...
	result, _ := app.DB.Exec("UPDATE _benmore_password_resets SET used = 1 WHERE token = ? AND used = 0 AND type = 'verify'", token)
	if affected, _ := result.RowsAffected(); affected == 0 {
		http.Redirect(w, r, app.Paths.Login, http.StatusSeeOther)
		return
	}
