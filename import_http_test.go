...
func TestImportCommitRecoveryInterleaveKeepsLiveStaging(t *testing.T) {
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatal("import did not reach a terminal state")
		}
...
// importCallerAuthorized) - see auth.go:493 validateCSRFReason and
// auth.go:363 authCSRFSign for the pairing this reproduces. Sending the
...
// column (db.go:599 EnsureUsersTable, auth.go:82 EnsureSessionsTable) -
// both would fail their INSERT outright. This version calls the real
// table-ensure functions and CreateSession (auth.go:220), the same helper
// production code uses, so the row shape can't drift from what
// getSession/GetSessionFromDB (auth.go:2417, auth.go:241) actually read.
...
// auth.go:485) ran and logged BEFORE either short-circuit had a chance to
