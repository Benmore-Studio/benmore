...
<title>`, lang))
	...
	if app != nil {
		sb.WriteString(RecorderScript(app, app.DevMode))
		// Testing mode: banner + feedback widget. Shown on every page
		// (including pre-auth /login, /signup) when features.testing is
		// opted in - the whole point of testing mode is to make every
		// view obviously non-production.
		if app.FeatureEnabled("testing") {
			sb.WriteString(TestingBannerHTML(session, app))
			sb.WriteString(FeedbackWidgetHTML())
		}
