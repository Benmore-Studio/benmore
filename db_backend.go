...
func sqliteAppDSN(dbPath, dir string) string {
	...
	return dbPath + "?_journal_mode=WAL" +
		...
		// WriteTimeout; platform_db.go and embed_dashboard.go already use 10000.
		"&_busy_timeout=10000" +
...
func sqliteSynchronousMode(dir string) string {
	raw := os.Getenv("DB_SYNCHRONOUS")
	if raw == "" {
		raw = GetEnv(dir, "DB_SYNCHRONOUS")
	}
	...
	case "EXTRA":
		return "EXTRA"
	default:
		// A non-empty value we don't recognize (typo like "FUL" or a numeric
		// "2") would otherwise silently fall through to NORMAL - a durability
		// downgrade with no signal. Warn so an operator who intended FULL/EXTRA
		// isn't left at NORMAL unknowingly.
		log.Printf("db: DB_SYNCHRONOUS=%q is not one of NORMAL|FULL|EXTRA; defaulting to NORMAL", raw)
		return "NORMAL"
	}
}
