...
// TestLoadFlowsGHA_APIJSONNestedArrays asserts that the GHA loader preserves
// nested arrays/objects in a `run: api` step's `with.json` block instead of
// flattening them to a map[string]string. This is the regression test for
// benmore-go #94 (Modern Treasury routing_details / account_details arrays).
func TestLoadFlowsGHA_APIJSONNestedArrays(t *testing.T) {
	dir := t.TempDir()
	yamlBody := `
on:
  request:
...
jobs:
  create_counterparty:
    steps:
      - id: x
        run: sql
        query: SELECT acct FROM accounts WHERE id = :id
          ...
          json:
            routing_details:
              - routing_number: "021000021"
                ...
                payment_type: ach
            account_details:
              - account_number: "${{ steps.x.outputs.acct }}"
...
`
	...
	f := flows[0]
	if len(f.Steps) != 2 {
		t.Fatalf("expected 2 steps, got %d: %+v", len(f.Steps), f.Steps)
	}

	apiStep := f.Steps[1]
	if apiStep.Type != "api" {
		t.Fatalf("step 1 should be api, got %q", apiStep.Type)
	}
	if apiStep.API == nil {
		t.Fatal("step 1 API is nil")
	}
	if apiStep.API.JSON == nil {
		t.Fatal("api.JSON is nil - nested json block was not parsed")
	}

	// routing_details must be a []any, not a flattened string. #94.
	rdRaw, ok := apiStep.API.JSON["routing_details"]
	if !ok {
		t.Fatal("api.JSON missing routing_details key")
	}
	rd, ok := rdRaw.([]any)
	if !ok {
		t.Fatalf("routing_details is %T, want []any (was flattened to string?)", rdRaw)
	}
	if len(rd) != 1 {
		t.Fatalf("routing_details len = %d, want 1", len(rd))
	}
	rdRow, ok := rd[0].(map[string]any)
	if !ok {
		t.Fatalf("routing_details[0] is %T, want map[string]any", rd[0])
	}
	if rdRow["routing_number"] != "021000021" {
		t.Errorf("routing_details[0].routing_number = %v, want 021000021", rdRow["routing_number"])
	}
	if rdRow["routing_number_type"] != "aba" {
		t.Errorf("routing_details[0].routing_number_type = %v, want aba", rdRow["routing_number_type"])
	}

	// account_details string-leaf ${{ }} ref must be normalized to {{ }} form. #94.
	adRaw, ok := apiStep.API.JSON["account_details"]
	if !ok {
		t.Fatal("api.JSON missing account_details key")
	}
	ad, ok := adRaw.([]any)
	if !ok {
		t.Fatalf("account_details is %T, want []any", adRaw)
	}
	if len(ad) != 1 {
		t.Fatalf("account_details len = %d, want 1", len(ad))
	}
	adRow, ok := ad[0].(map[string]any)
	if !ok {
		t.Fatalf("account_details[0] is %T, want map[string]any", ad[0])
	}
	acctNum, _ := adRow["account_number"].(string)
	if acctNum != "{{x.acct}}" {
		t.Errorf("account_number ref not normalized: got %q, want {{x.acct}}", acctNum)
	}

	// metadata is a map, not flattened. ein_id ref also normalized.
	metaRaw, ok := apiStep.API.JSON["metadata"]
	if !ok {
		t.Fatal("api.JSON missing metadata key")
	}
	meta, ok := metaRaw.(map[string]any)
	if !ok {
		t.Fatalf("metadata is %T, want map[string]any", metaRaw)
	}
