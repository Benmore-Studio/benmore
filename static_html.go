...
func tryServeHTMLFile(w http.ResponseWriter, r *http.Request, app *App, path string) bool {
	...
	if app != nil {
		...
		// path - so HTML-stack apps silently lost the widget. Inject it
		// explicitly here, gated on FeedbackWidgetEnabled() so it shows in
		// testing mode AND on published apps running features.feedback.
		if app.FeedbackWidgetEnabled() {
			injected = injectFeedbackWidget(injected, app.FeatureEnabled("testing"))
		}
...
func injectFeedbackWidget(html []byte, testing bool) []byte {
	if bytes.Contains(html, []byte(`id="bm-feedback-btn"`)) {
		return html
	}
	chunk := []byte("\n" + FeedbackWidgetHTML(testing) + "\n")
	lower := bytes.ToLower(html)
	if i := bytes.LastIndex(lower, []byte("</body>")); i >= 0 {
		out := make([]byte, 0, len(html)+len(chunk))
		out = append(out, html[:i]...)
...
var staticAssetRefRe = regexp.MustCompile(
	`(?i)(href|src)=("|')(/static/[^"'?]+?\.(?:css|js|mjs|cjs|png|jpg|jpeg|gif|webp|svg|avif|ico|woff2|woff|ttf|otf|eot))(\?v=[0-9a-zA-Z]+)?("|')`)

// versionStaticAssets rewrites `href="/static/foo.css"` and
// `src="/static/foo.js"` references in served HTML to include a
// `?v=<content-hash>` query string. This invalidates Cloudflare's immutable
// cache (which the static handler sets to max-age=1y) the moment a file's
// CONTENT changes - no agent has to remember to bump a version in their HTML,
// and no `?v=N`-thrash debugging loops. A content hash (not the bare mtime)
// means a `git revert` that restores older bytes serves the matching cached
// version instead of a stale one keyed on a reused mtime.
//
// Idempotent + framework-owned: a prior `?v=<hash>` we emitted is replaced
// with the current one (the regex matches `?v=<alnum>`); a non-`v` query
// string (author intent) is left untouched, as is a ref whose file doesn't
// exist on disk (so a real 404 surfaces normally).
//
...
func versionStaticAssets(html []byte, app *App) []byte {
	...
	return staticAssetRefRe.ReplaceAllFunc(html, func(match []byte) []byte {
		...
		ver, ok := assetContentVersion(staticRoot, rel)
		if !ok {
			return match
		}
...
// assetVerCache memoizes the content-hash version of a /static/ asset keyed on
// (rel, mtime, size) so the hot serve path only re-hashes a file when it
// actually changes. The cache value is a CONTENT hash, so correctness never
// depends on the mtime - the key is just a cheap "did this change" probe.
var (
	assetVerMu    sync.Mutex
	assetVerCache = map[string]assetVerEntry{}
)

// assetVerEntry caches an asset's content hash (ver) for its current change-
// probe (mtime+size, or the source-tree fingerprint). Keyed on the STABLE asset
// id (the rel path / staticRoot) so a new probe OVERWRITES the old entry rather
// than appending - bounding the cache to one entry per asset/tree on a
// long-lived multi-tenant process (the old (rel,mtime,size) key grew without
// bound, one entry per edit).
type assetVerEntry struct{ probe, ver string }

// assetContentVersion returns a short content-hash cache-buster for a
// /static/<rel> reference. Using a content hash (not the bare mtime) fixes the
// post-revert stale-bundle footgun: a `git revert` can restore older bytes
// under a reused mtime, so an mtime-keyed `?v=` served the STALE cached copy;
// a content hash changes iff the bytes change, and identical bytes reuse the
// correct cached version. Memoized per (rel, mtime, size).
//
// For .js - whether plain on-disk or JIT-compiled from a sibling .tsx/.ts/.jsx
// - there's no stable compiled artifact to hash, so we hash the SOURCE tree
// (every .tsx/.ts/.jsx plus any on-disk .js). Editing any imported module
// changes the hash, the same broad signal the old mtime path used.
func assetContentVersion(staticRoot, rel string) (string, bool) {
	if strings.HasSuffix(rel, ".js") {
		return staticSourceTreeVersion(staticRoot)
	}
	...
	probe := strconv.FormatInt(st.ModTime().UnixNano(), 10) + "|" + strconv.FormatInt(st.Size(), 10)
	if v, ok := cachedAssetVer(rel, probe); ok {
		return v, true
	}
	...
	storeAssetVer(rel, probe, v)
	return v, true
}

// staticSourceTreeVersion hashes every .tsx/.ts/.jsx/.js file under staticRoot
// (sorted by relpath, content included) into one version, memoized by the
// tree's (max-mtime, total-size, file-count). Bounded - apps have dozens of
// source files, and the memo means the walk+hash only runs after an edit.
func staticSourceTreeVersion(staticRoot string) (string, bool) {
	type srcFile struct {
		rel  string
		path string
	}
	...
	_ = filepath.Walk(staticRoot, func(path string, info os.FileInfo, err error) error {
		...
		if strings.HasSuffix(name, ".tsx") || strings.HasSuffix(name, ".ts") ||
			strings.HasSuffix(name, ".jsx") || strings.HasSuffix(name, ".js") {
			rel, _ := filepath.Rel(staticRoot, path)
			files = append(files, srcFile{rel: rel, path: path})
			if m := info.ModTime().UnixNano(); m > maxMtime {
				maxMtime = m
			}
			totalSize += info.Size()
		}
	...
	stable := "__tree__|" + staticRoot
	probe := strconv.FormatInt(maxMtime, 10) + "|" +
		strconv.FormatInt(totalSize, 10) + "|" +
		strconv.Itoa(len(files))
	if v, ok := cachedAssetVer(stable, probe); ok {
		return v, true
	}
	...
	storeAssetVer(stable, probe, v)
	return v, true
}

// cachedAssetVer returns the cached version for stable id `stable` IFF its
// stored probe still matches `probe` (i.e. the asset hasn't changed).
func cachedAssetVer(stable, probe string) (string, bool) {
	assetVerMu.Lock()
	defer assetVerMu.Unlock()
	if e, ok := assetVerCache[stable]; ok && e.probe == probe {
		return e.ver, true
	}
	return "", false
}

func storeAssetVer(stable, probe, ver string) {
	assetVerMu.Lock()
	assetVerCache[stable] = assetVerEntry{probe: probe, ver: ver}
	assetVerMu.Unlock()
}
