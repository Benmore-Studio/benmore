...
type FlowEmailYAML struct {
	...
	Body     string            `yaml:"body"` // alias for html
	Text     string            `yaml:"text"`
	Data     map[string]string `yaml:"data"`
	Vars     map[string]string `yaml:"vars"` // alias for data
}
...
// LoadBackupConfig is the compatibility wrapper; runtime uses loadRuntimeConfig.
func LoadBackupConfig(dir string) *BackupConfig {
	...
	_, config, err := parseAppConfigDocument(data)
	if err != nil {
		log.Printf("%s", err)
	}
	return config
...
func LoadAppConfigYAML(dir string) *DesignConfig {
	...
	raw, _, err := parseAppConfigDocument(data)
	if err != nil {
		log.Printf("%s", err)
		return nil
	}
	...
	return designFromAppYAML(raw)
}

func designFromAppYAML(raw map[string]any) *DesignConfig {
	config := &DesignConfig{
	...
	// Top-level site_name is the scaffold's canonical form; without this
	// it never reached Design.SEO, so auth-email subjects and the per-app
	// OAuth metadata fell back to "App". seo.site_name still wins below.
	if v, ok := raw["site_name"].(string); ok && v != "" {
		config.SEO["site_name"] = v
	}
...
func parseFeaturesConfig(m map[string]any) *FeaturesConfig {
	...
	fc.Feedback = parseBoolFeature("feedback")
...
func LoadGroupConfig(dir string) *GroupConfig {
	...
	gc := &GroupConfig{Table: table, Key: key, UserField: userField}
	if rf, _ := m["role_field"].(string); rf != "" {
		gc.RoleField = rf
	}
	if b, ok := m["bootstrap"].(map[string]any); ok {
		orgTable, _ := b["org_table"].(string)
		foundingRole, _ := b["founding_role"].(string)
		if orgTable != "" {
			gc.Bootstrap = &GroupBootstrapConfig{OrgTable: orgTable, FoundingRole: foundingRole}
		}
	}
	return gc
}

// LoadWSRooms reads the `ws_rooms:` section from app.yaml (v2.7.164):
// room-name patterns bound to membership-table checks, enforced at WS
// join time. Malformed rules are dropped with a startup warning so a
// typo can't silently leave a room open.
func LoadWSRooms(dir string) []WSRoomRule {
	path := filepath.Join(dir, "app.yaml")
	data, err := os.ReadFile(path)
	if err != nil {
		return nil
	...
	m, ok := raw["ws_rooms"].(map[string]any)
	if !ok {
		return nil
	}
	var rules []WSRoomRule
	for pattern, v := range m {
		ruleStr, _ := v.(string)
		rule := parseMemberOf("member-of:" + strings.ToLower(strings.TrimSpace(ruleStr)))
		if pattern == "" || rule == nil {
			log.Printf("WARNING: ws_rooms.%q = %q is malformed (want `<table>(<join_col>, <user_col>)`); rule dropped", pattern, ruleStr)
			continue
		}
		rules = append(rules, WSRoomRule{Pattern: pattern, Rule: rule})
	}
	// Deterministic match order: most specific (longest) pattern first.
...
func LoadHooksYAML(dir string) *HookConfig {
	...
	config, err := parseHooksYAML(data)
	if err != nil {
		log.Printf("%s", err)
	}
	return config
...
func parseHooksYAML(data []byte) (*HookConfig, error) {
	if msg := validateHooksYAML(string(data)); msg != "" {
	...
	if len(raw.OnInsert)+len(raw.OnUpdate)+len(raw.OnDelete)+len(raw.BeforeInsert)+len(raw.BeforeUpdate)+len(raw.BeforeDelete) == 0 {
		return nil, nil
	}
	config := &HookConfig{
...
// LoadFlowsYAML retains supported legacy root flows alongside GHA files.
func LoadFlowsYAML(dir string) []Flow {
	flows, err := loadFlowsChecked(dir, true)
	if err != nil {
		log.Printf("%s", err)
	}
	return flows
...
func LoadWorkflowsYAML(dir string) *WorkflowConfig {
	config, err := loadWorkflowsYAMLChecked(dir)
	if err != nil {
...
// loadWorkflowsYAMLChecked distinguishes intentional removal from invalid input.
// Runtime startup and reload must propagate its error before changing app state.
func loadWorkflowsYAMLChecked(dir string) (*WorkflowConfig, error) {
	data, err := os.ReadFile(filepath.Join(dir, "workflows.yaml"))
	if os.IsNotExist(err) {
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("read workflows.yaml: %w", err)
	}
	return parseWorkflowsYAML(data)
}

func parseWorkflowsYAML(data []byte) (*WorkflowConfig, error) {
