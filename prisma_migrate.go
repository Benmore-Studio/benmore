...
func ApplyPrismaMigration(db *sql.DB, dir, prismaSrc string) (string, error) {
	...
	rollback := func(cause error) (string, error) {
		...
		// the new migration succeeds. If not, ANOTHER .failed file
		// accumulates — but at that point the user can `ls migrations/`
		// and see exactly what's been tried, which beats silent retry.
		failedPath := filePath + ".failed"
		if mvErr := os.Rename(filePath, failedPath); mvErr != nil {
