...
func ProviderGapWarnings(app *App) []string {
	...
	// Built-in OAuth providers activate from per-app <NAME>_CLIENT_ID and
	// <NAME>_CLIENT_SECRET values. Custom metadata uses top-level oauth in
	// app.yaml; nested auth.oauth is invalid. Missing credentials skip route
	// registration, so inspect configured providers when diagnosing absent login.
